...
class SpaceDropDownPanel extends Component<SpaceDropDownPanelProps, SpaceDropDownState> {
    ...
    handleSearchRelayInput = async (e: Event) => {
        await setState(this, {
            searchSpaceValue: (e.target as HTMLInputElement).value,
        });
    };
...
export type func_GetConversationList = (space?: URL) => Array<ConversationSummary>;
...
class ConversationList extends Component<ConversationListProps> {
    render(props: ConversationListProps) {
        ...
        const pinned = [];
        const unpinned = [];
        const currentSpaceConversationList = props.getters.getConversationList(props.currentSpace);
...
class UserIndicator extends Component<UserIndicatorProps> {
    render(props: UserIndicatorProps) {
        return (
            <div class="w-full">
                <div
                    class="hover:bg-neutral-800 rounded-md py-1 px-2 flex items-center justify-start gap-2 cursor-pointer"
                    onClick={() => {
                        props.emit({
                            type: "ShowProfileSetting",
                        });

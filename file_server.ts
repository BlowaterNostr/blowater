...
import { posixJoin } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/_join.ts";
import { posixNormalize } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/_normalize.ts";
import { extname } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/extname.ts";
import { join } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/join.ts";
import { relative } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/relative.ts";
import { resolve } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/resolve.ts";
import { SEP_PATTERN } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../path/separator.ts";
import { contentType } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../media_types/content_type.ts";
import {
    calculate,
    ifNoneMatch,
} from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/./etag.ts";
import {
    isRedirectStatus,
    Status,
} from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/./http_status.ts";
import { ByteSliceStream } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../streams/byte_slice_stream.ts";
import { parse } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../flags/mod.ts";
import { red } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../fmt/colors.ts";
import { createCommonResponse } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/./util.ts";
import { VERSION } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../version.ts";
import { format as formatBytes } from "https://raw.githubusercontent.com/denoland/deno_std/0.202.0/http/../fmt/bytes.ts";
...
const ENV_PERM_STATUS = Deno.permissions.querySync?.({ name: "env", variable: "DENO_DEPLOYMENT_ID" })
...
function main() {
    ...
    const rate_limit = new Map<string, { time: number; count: number }>();

    const handler: Deno.ServeHandler = async (
        req: Request,
        info: Deno.ServeHandlerInfo,
    ): Promise<Response> => {
        console.log(info.remoteAddr, rate_limit);
        const last_seen = rate_limit.get(info.remoteAddr.hostname);
        if (last_seen) {
            if (Date.now() - last_seen.time < 1000 && last_seen.count <= 6) {
                rate_limit.set(info.remoteAddr.hostname, {
                    time: last_seen.time,
                    count: last_seen.count + 1,
                });
            } else {
            ...
            setTimeout(() => {
                rate_limit.delete(info.remoteAddr.hostname);
            }, 1000);
        }
        return serveDir(req, {
            fsRoot: target,
